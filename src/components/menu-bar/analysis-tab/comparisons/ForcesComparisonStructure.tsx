...
export function ForcesComparisonStructure({ onClose }: Props) {
  ...
  }, [labels, mxSTAAD, mySTAAD, mzSTAAD, mxIDS, myIDS, mzIDS]);
  ...
  const handleApiCall = async () => {
    ...
    try {
      ...
      if (!targetSheet) {
        console.log('The "Global  Element Forces Comparison" section could not be found in any sheet.');
      ...
      for (const row of targetSheet.data.slice(1) as string[][]) {
        ...
        /*console.log("this is the extracted data",row);*/
      ...
      dispPipeCheckData.forEach(row => {
        ...
        tempFxSTAAD.push(row[10]);
        tempFySTAAD.push(row[11]);
        tempFzSTAAD.push(row[12]);
        tempMxSTAAD.push(row[13]);
        tempMySTAAD.push(row[14]);
        tempMzSTAAD.push(row[15]);
      });
