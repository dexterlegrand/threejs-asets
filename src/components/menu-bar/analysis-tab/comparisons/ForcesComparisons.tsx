...
export function ForcesComparisons({ onClose }: Props) {
  ...
  const modelLC = useMemo(() => {
    ...
    for (const model of models) {
      const checks = ui?.analysisUI[model]?.memberEndForces;
      if (!checks) continue;
      map.set(
        model,
        checks.map((c) => c.LCNumber)
  ...
  const forces = useMemo(() => {
    ...
    const analysis = line
      ...
      : undefined;
    if (!analysis) return [];
    return analysis.memberEndForces.filter((el) => `${el.LCNumber}` === LC);
  ...
  const options1 = {
    scales: {
      xAxes: [{
        scaleLabel: {
          display: true,
          labelString: 'Node No. - Element No.'
        }
      }],
      yAxes: [{
        scaleLabel: {
  ...
  }, [labels, mxCII, myCII, mzCII, mxIDS, myIDS, mzIDS]);
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const dispPipeCheckIndex = (jsonSheet as string[][]).findIndex(
          (row: string[]) => row.some((cell: string) => cell === 'Global  Element Forces Comparison')
        );
        if (dispPipeCheckIndex !== -1) {
          console.log(`"Global  Element Forces Comparison" found in sheet: ${sheetName}`);
          targetSheet = {
            name: sheetName,
      ...
      if (!targetSheet) {
        console.log('Global  Element Forces Comparison" section could not be found in any sheet.');
        alert('The "Global  Element Forces Comparison" section could not be found in any sheet.');
        return;
      }

      const tempelIDS: string[] = [];
      ...
      for (const row of targetSheet.data.slice(1) as string[][]) {
        ...
        if (row.every((cell: string) => cell === 'Global Element Forces Deviation')) {
          //console.log('Ending data extraction.');
          break;
        }
        console.log("This is the extracted data from the force comparison table",row);
        dispPipeCheckData.push(row);
      }

      dispPipeCheckData.forEach(row => {
        /*nodeIDS.push(row[0]);*/
        tempnodeIDS.push(`${row[1]}-${row[0]}`);
        tempFxIDS.push(row[2]);
        tempFyIDS.push(row[3]);
        tempFzIDS.push(row[4]);
        tempMxIDS.push(row[5]);
        tempMyIDS.push(row[6]);
        tempMzIDS.push(row[7]);
        /*nodeCII.push(row[7]);*/
        tempFxCII.push(row[10]);
        tempFyCII.push(row[11]);
        tempFzCII.push(row[12]);
        tempMxCII.push(row[13]);
        tempMyCII.push(row[14]);
        tempMzCII.push(row[15]);
      });
