...
export function EquipmentOperatingLoadComparisonStructure({ onClose }: Props) {
...
//   }, [line, model, lineLC, modelLC]);
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const dispPipeCheckIndex = (jsonSheet as string[][]).findIndex(
          (row: string[]) => row.some((cell: string) => cell === 'Equipment Operating Load Check')
        );
        if (dispPipeCheckIndex !== -1) {
          console.log(`"Equipment Operating Load Check: ${sheetName}`);
          targetSheet = {
            name: sheetName,
      ...
      if (!targetSheet) {
        console.log('Equipment Operating Load Check" section could not be found in any sheet.');
        alert('The "Equipment Operating Load Check" section could not be found in any sheet.');
        return;
      }
  ...
  return (
    <CustomDlg
      ...
      isMinimize={true}
      position={"center"}
      title={"Equipment Operating Load Comparison"}
      body={
