...
export function PipingOperatingLoadComparisonStructure({ onClose }: Props) {
...
//   }, [line, model, lineLC, modelLC]);
    ...
    try {
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const dispPipeCheckIndex = (jsonSheet as string[][]).findIndex(
          (row: string[]) => row.some((cell: string) => cell === 'Piping Operating Load Check')
        );
        if (dispPipeCheckIndex !== -1) {
          console.log(`"Piping Operating Load Check: ${sheetName}`);
          targetSheet = {
            name: sheetName,
      ...
      if (!targetSheet) {
        console.log('Piping Operating Load Check" section could not be found in any sheet.');
        alert('The "Piping Operating Load Check" section could not be found in any sheet.');
        return;
      }
  ...
  return (
    <CustomDlg
      ...
      isMinimize={true}
      position={"center"}
      title={"Piping Operating Load Comparison"}
      body={
