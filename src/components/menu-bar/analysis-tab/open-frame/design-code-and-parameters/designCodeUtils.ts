...
function getElementLengthsOF(model: TOpenFrame, elements: TBeamElement[], el: TBeamElement) {
...
function getDeflectionLengthsOF(elements: TBeamElement[], el: TBeamElement): number {
...
export function getOFEffectiveLengths(
  ui: OpenFrameUI, 
  ...
  const all = splitElements(models, ui.loadingsUI);
  for (const model of models) {
    const elements = all.get(model.name);
    if (!elements) continue;
    const json = getJSONForDesignCodesAndParametersOF(ui, designCode, scene, project, model, models);
    for (const elem of elements) {
      const member: Member = json.members.find((m: Member) => m.name === elem.name);
      const { Ky, Kz, Ly, Lz } = getElementLengthsOF(model, elements, elem);
      els.push({
        id: els.length,
        selected: false,
        model: model.name,
        element: elem.name,
        elementNo: member?.label,
        Ky,
        Kz,
        Ly: roundM(Ly),
        Lz: roundM(Lz),
...
export function getOFDeflectionLengths(models: TOpenFrame[], loadings: LoadingsUI) {
  ...
  for (const model of models) {
    ...
    for (const elem of elements) {
      els.push({
        ...
        model: model.name,
        element: elem.name,
        dl: getDeflectionLengthsOF(elements, elem),
      });
    }
  }
  return els;
