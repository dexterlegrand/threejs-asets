...
export function TankSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessTank[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.TANK) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.tankSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          tankSummary: {
            ...process.analysis.tankSummary,
            // @ts-ignore
            [field]: { ...process.analysis.tankSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessTank, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("TANK")) : [];
      const entries = Object.entries(data);
      let tankSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, tankSummary },
        })
      );
    });
  }
