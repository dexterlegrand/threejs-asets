...
export function ExpanderSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessExpander[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.EXPANDER) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.expanderSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          expanderSummary: {
            ...process.analysis.expanderSummary,
            // @ts-ignore
            [field]: { ...process.analysis.expanderSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessExpander, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("EXPANDER")) : [];
      const entries = Object.entries(data);
      let expanderSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, expanderSummary },
        })
      );
    });
  }
