...
export function ExtractorSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessExtractor[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.EXTRACTOR) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.extractorSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          extractorSummary: {
            ...process.analysis.extractorSummary,
            // @ts-ignore
            [field]: { ...process.analysis.extractorSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessExtractor, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("EXTRACTOR")) : [];
      const entries = Object.entries(data);
      let extractorSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, extractorSummary },
        })
      );
    });
  }
