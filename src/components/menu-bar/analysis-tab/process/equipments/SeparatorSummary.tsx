...
export function SeparatorSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessSeparator[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.SEPARATOR) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.separatorSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          separatorSummary: {
            ...process.analysis.separatorSummary,
            // @ts-ignore
            [field]: { ...process.analysis.separatorSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessSeparator, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("SEPARATOR")) : [];
      const entries = Object.entries(data);
      let separatorSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, separatorSummary },
        })
      );
    });
  }
