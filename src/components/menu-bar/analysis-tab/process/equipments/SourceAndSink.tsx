...
export function SourceAndSink({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: (TProcessSource | TProcessSink)[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.SOURCE || el.type === EProcessElementType.SINK)
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.sourceAndSinkSummary;
  }, [current, state]);
      ...
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...
          sourceAndSinkSummary: {
            ...process.analysis.sourceAndSinkSummary,
            // @ts-ignore
            [field]: { ...process.analysis.sourceAndSinkSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessSource | TProcessSink, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0]
        ? Object.keys(rows[0]).filter((d) => d.includes("SOURCE") || d.includes("SINK"))
        : [];
      const entries = Object.entries(data);
      let sourceAndSinkSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, sourceAndSinkSummary },
        })
      );
    });
  }
