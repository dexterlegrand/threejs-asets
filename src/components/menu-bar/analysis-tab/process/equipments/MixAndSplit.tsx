...
export function MixAndSplit({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: (TProcessMix | TProcessSplit)[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.MIX || el.type === EProcessElementType.SPLIT)
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.mixerAndSplitSummary;
  }, [current, state]);
      ...
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...
          mixerAndSplitSummary: {
            ...process.analysis.mixerAndSplitSummary,
            // @ts-ignore
            [field]: { ...process.analysis.mixerAndSplitSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessMix | TProcessSplit, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0]
        ? Object.keys(rows[0]).filter((d) => d.includes("MIX") || d.includes("SPLIT"))
        : [];
      const entries = Object.entries(data);
      let mixerAndSplitSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, mixerAndSplitSummary },
        })
      );
    });
  }
