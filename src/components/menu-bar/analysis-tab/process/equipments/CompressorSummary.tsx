...
export function CompressorSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessCompressor[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.COMPRESSOR) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.compressorSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          compressorSummary: {
            ...process.analysis.compressorSummary,
            // @ts-ignore
            [field]: { ...process.analysis.compressorSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessCompressor, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, compressorSummary },
        })
      );
    });
  }
