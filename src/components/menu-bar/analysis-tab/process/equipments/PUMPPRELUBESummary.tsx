...
export function PUMPPRELUBESummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessPUMPPRELUBE[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.PUMP_PRELUBE) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.PUMPPRELUBESummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          PUMPPRELUBESummary: {
            ...process.analysis.PUMPPRELUBESummary,
            // @ts-ignore
            [field]: { ...process.analysis.PUMPPRELUBESummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessPUMPPRELUBE, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, PUMPPRELUBESummary},
        })
      );
    });
  }
