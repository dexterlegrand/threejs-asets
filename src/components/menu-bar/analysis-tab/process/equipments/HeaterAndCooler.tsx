...
export function HeaterAndCooler({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: (TProcessHeater | TProcessCooler)[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.HEATER || el.type === EProcessElementType.COOLER)
        items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.heaterAndCoolerSummary;
  }, [current, state]);
      ...
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...
          heaterAndCoolerSummary: {
            ...process.analysis.heaterAndCoolerSummary,
            // @ts-ignore
            [field]: { ...process.analysis.heaterAndCoolerSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessHeater | TProcessCooler, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0]
        ? Object.keys(rows[0]).filter((d) => d.includes("HEATER") || d.includes("COOLER"))
        : [];
      const entries = Object.entries(data);
      let heaterAndCoolerSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, heaterAndCoolerSummary },
        })
      );
    });
  }
