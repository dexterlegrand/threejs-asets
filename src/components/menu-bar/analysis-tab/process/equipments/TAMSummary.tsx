...
export function TAMSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessTAM[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.TAM) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.TAMSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          TAMSummary: {
            ...process.analysis.TAMSummary,
            // @ts-ignore
            [field]: { ...process.analysis.TAMSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessTAM, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, TAMSummary},
        })
      );
    });
  }
