...
export function HeaderSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessHeader[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.HEADER) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.headerSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          headerSummary: {
            ...process.analysis.headerSummary,
            // @ts-ignore
            [field]: { ...process.analysis.headerSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessHeader, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("HEADER")) : [];
      const entries = Object.entries(data);
      let headerSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, headerSummary },
        })
      );
    });
  }
