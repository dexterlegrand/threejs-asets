...
export function ACSummary({ onClose }: Props) {
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.ACSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          ACSummary: {
            ...process.analysis.ACSummary,
            // @ts-ignore
            [field]: { ...process.analysis.ACSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, ACSummary},
        })
      );
    });
  }
