...
export function NOX_ABATORSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessNox_Abator[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.NOX_ABATOR) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.Nox_AbatorSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          Nox_AbatorSummary: {
            ...process.analysis.Nox_AbatorSummary,
            // @ts-ignore
            [field]: { ...process.analysis.Nox_AbatorSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, Nox_AbatorSummary},
        })
      );
    });
  }
