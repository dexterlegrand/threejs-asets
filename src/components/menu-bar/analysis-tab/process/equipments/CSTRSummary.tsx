...
export function CSTRSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessCSTR[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.CSTR) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.CSTRSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          CSTRSummary: {
            ...process.analysis.CSTRSummary,
            // @ts-ignore
            [field]: { ...process.analysis.CSTRSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessCSTR, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("CSTR")) : [];
      const entries = Object.entries(data);
      let CSTRSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, CSTRSummary },
        })
      );
    });
  }
