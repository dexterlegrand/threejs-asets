...
export function DrumSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessDrum[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.DRUM) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.drumSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          drumSummary: {
            ...process.analysis.drumSummary,
            // @ts-ignore
            [field]: { ...process.analysis.drumSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessDrum, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("DRUM")) : [];
      const entries = Object.entries(data);
      let drumSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, drumSummary },
        })
      );
    });
  }
