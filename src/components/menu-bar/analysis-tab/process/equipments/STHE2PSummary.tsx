...
export function STHE2PSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessSTHE2P[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.ST_HE_2P) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.STHE2PSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          STHE2PSummary: {
            ...process.analysis.STHE2PSummary,
            // @ts-ignore
            [field]: { ...process.analysis.STHE2PSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessSTHE2P, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, STHE2PSummary },
        })
      );
    });
  }
  ...
  return (
    <CustomDlg
      title={"SP Heat exchanger two passtube-side Summary"}
      isMinimize={true}
      body={
