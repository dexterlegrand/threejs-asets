...
export function AHSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessAH[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.AH) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.AHSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          AHSummary: {
            ...process.analysis.AHSummary,
            // @ts-ignore
            [field]: { ...process.analysis.AHSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessAH, field: string, value: any) {
    dispatch(
