...
export function PSVSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessPSV[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.PSV) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.PSVSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          PSVSummary: {
            ...process.analysis.PSVSummary,
            // @ts-ignore
            [field]: { ...process.analysis.PSVSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessPSV, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, PSVSummary },
        })
      );
    });
  }
