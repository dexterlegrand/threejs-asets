...
export function DAFSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessDAF[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.DAF) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.DAFSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          DAFSummary: {
            ...process.analysis.DAFSummary,
            // @ts-ignore
            [field]: { ...process.analysis.DAFSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessDAF, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis,DAFSummary},
        })
      );
    });
  }
