...
export function EnlargerSummary({ onClose }: Props) {
  ...
  const items = useMemo(() => {
    ...
    if (!process) return [];
    const items: TProcessEnlarger[] = [];
    for (const el of Array.from(process.elements.values())) {
      if (el.type === EProcessElementType.ENLARGER) items.push(el as any);
    }
    return items;
  }, [current, state]);
  ...
  const data = useMemo(() => {
    const process = state.processes.get(current);
    return process?.analysis.enlargerSummary;
  }, [current, state]);
    ...
    dispatch(
      changeProcessAnalysisAction(current, {
        ...
        analysis: {
          ...process.analysis,
          enlargerSummary: {
            ...process.analysis.enlargerSummary,
            // @ts-ignore
            [field]: { ...process.analysis.enlargerSummary[field], UOM },
          },
        },
      })
    );
  ...
  function handleChange(item: TProcessEnlarger, field: string, value: any) {
    dispatch(
  ...
  function handleImport() {
    ...
    importFromCSV((rows) => {
      ...
      const names = rows[0] ? Object.keys(rows[0]).filter((d) => d.includes("ENLARGER")) : [];
      const entries = Object.entries(data);
      let enlargerSummary = { ...data };
      ...
      dispatch(
        changeProcessAnalysisAction(current, {
          ...process,
          analysis: { ...process.analysis, enlargerSummary },
        })
      );
    });
  }
