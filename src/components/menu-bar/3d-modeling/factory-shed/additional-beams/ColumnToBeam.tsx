...
import { Project, SimpleDirection, Releases } from "../../../../../store/main/types";
...
const ColumnToBeam: FunctionComponent<Props> = (props) => {
  ...
  function handleSoftChange(row: OFColumnToBeamUI, field: "profile", value: any) {
    if (!openFrameUI) return;
  ...
  function handleHardChange(row: OFColumnToBeamUI, field: "model" | "column" | "beam", value: any) {
    if (!openFrameUI) return;
    handleChangeRow(row, field, value);
    const changedUI = { ...row, [field]: value };
  ...
  function handleImport() {
    ...
    importFromCSV((newData, isCSV) => {
      ...
      const newBeams: OFColumnToBeamUI[] = [];
