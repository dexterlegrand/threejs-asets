...
const FramesGeometry: FunctionComponent<Props> = (props) => {
  ...
  function recreatingModel(project: Project, ui: OpenFrameUI) {
    const modelName = `FS${getIndexName(
      getFSModels(project).filter(
        (model) =>
    ...
    dispatch(
      changeProjectAction({
        ...
        models: [
          ...
          createFSModel(modelName, project.name, newUI),
        ],
      })
    );
    dispatch(changeOFUIAction(newUI));
    if (newUI.frames.material) {
      dispatch(setModelTypeAndMaterial(newUI.frames.material, "Factory Shed"));
    }
  }

  function creatingModel(project: Project, ui: OpenFrameUI) {
    const modelName = `FS${getIndexName(
      project.models.filter((model) => model.type === "Factory Shed"),
      "FS"
    )}`;
    const newUI: OpenFrameUI = {
      ...ui,
    ...
    dispatch(createModel(createFSModel(modelName, project.name, newUI)));
    dispatch(changeOFUIAction(newUI));
    if (ui.frames.material) {
      dispatch(setModelTypeAndMaterial(ui.frames.material, "Factory Shed"));
    }
  }
