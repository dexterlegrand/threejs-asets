      ...
      arr.forEach((item) => {
        try {
          ...
          if (itemModel) {
            ...
            if (!model)
              throw new Error(`Column Relocation (Import): Model "${itemModel}" not found`);
          }
          if (itemFrame) {
            frame = getElementByName(model?.frames, itemFrame);
            if (!frame)
              throw new Error(`Column Relocation (Import): Frame "${itemFrame}" not found`);
          }
          if (itemColumn) {
            column = model?.columns.find((column) => column.name === itemColumn);
            if (!column) throw new Error(`Relocation (Import): Column "${itemColumn}" not found`);
          }
          if (itemDistance) {
            if (!isNumber(itemDistance)) {
              throw new Error(
                `Column Relocation (Import): (id: ${item.id}) - a value of "Distance (m)" "${itemDistance}" is not a number!`
              );
            }
          }
          try {
