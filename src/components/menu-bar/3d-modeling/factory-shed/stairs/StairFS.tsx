...
import { Project, Orientation, SimpleDirection } from "../../../../../store/main/types";
import { useDispatch, useSelector } from "react-redux";
import { ApplicationState } from "../../../../../store";
import { removeConnections, updateConnections, getFSModels } from "../../../../3d-models/openFrame";
...
import { orientations, supportTypes } from "../../../../../store/main/constants";
...
export function StairFS(props: Props) {
  ...
  function handleCreateElements(
    ...
    const isToX = (getSimpleDirection(to.startPos, to.endPos) as SimpleDirection) === "X";
    if (from !== "Ground") {
      ...
      if (!row.fromDetails.vertical || !row.fromDetails.horizontal) return model;
    ...
    const el1: TStaircaseOF = {
      ...
      endPos: isToX ? endPos.clone().setX(endPos.x - width) : endPos.clone().setZ(endPos.z - width),
      profile: row.profile!,
      orientation: row.orientation,
      position: "L",
      direction,
      toX: isToX,
      supportType: row.from === "Ground" ? row.fromDetails.supportType : undefined,
      startConnected: [],
      connected: [],
      endConnected: [],
    };
  ...
  function handleChangeRow(row: OFStaircaseUI, field: string, value: any, isHard?: boolean) {
    ...
    const from =
      changedRow.from !== "Ground"
        ? getElementByName([...model.beams, ...model.cantilevers], changedRow.from)
        : "Ground";
    const to = getElementByName([...model.beams, ...model.cantilevers], changedRow.to);
    if (!from || !to) return;
    if (isHard) {
  ...
  function getRow(row: OFStaircaseUI) {
    ...
    const from = row.from !== "Ground" ? getElementByName(beams, row.from) : row.from;
    const toBeams =
      from !== undefined
        ? from !== "Ground"
          ? beams.filter(
              (beam) => from.direction === beam.direction && beam.startPos.y > from.startPos.y
            )
          : beams
        : [];
    ...
    return (
      <tr key={row.id}>
          ...
          <td>
            <Tooltip
              ...
              content={
                <div>
                  <p>
                    Vertical Distance to be Covered: <strong>{row.fromDetails.vertical}m</strong>
                  </p>
                  <p>
        ...
        <SelectorCell<Section>
          items={profiles.filter((profile) => profile.country_code === row.library)}
      ...
      for (const item of imported) {
        ...
        if (item.model) {
          ...
          if (model) {
            ...
            from =
              item.from !== "Ground"
                ? getElementByName([...model.beams, ...model.cantilevers], item.from)
                : "Ground";
            if (from) {
              newItem = {
                ...newItem,
            ...
            to = getElementByName([...model.beams, ...model.cantilevers], item.to);
            if (to) {
        ...
        const profile = getImportProfileByDesignation(profiles, item.profile, () =>
          showErrorMsg(`(id: ${item.id}) - a profile "${item.profile}" not found!`)
        );
        newItem = {
          ...newItem,
          fromDetails: {
        ...
        if (item.orientation) {
          if (orientations.includes(item.orientation)) {
            ...
            showErrorMsg(`(id: ${item.id}) - Incorrect orientation value (${item.orientation})!`);
          }
        }
        if (item.supportType) {
          if (supportTypes.includes(item.supportType)) {
            ...
            showErrorMsg(`(id: ${item.id}) - Incorrect support type (${item.supportType})!`);
          }
        }
        ...
        if (model && from && to && newItem.profile) {
          ...
          changedProject = {
            ...
            models: models.map((mItem) =>
              mItem.name === model!.name ? handleCreateElements(newItem, model!, from!, to!) : mItem
            ),
          };
        }
      }
