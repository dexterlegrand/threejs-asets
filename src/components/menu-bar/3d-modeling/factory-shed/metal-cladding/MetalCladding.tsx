...
import { TOpenFrame, TMetalCladdingOF, TColumnOF } from "../../../../../store/main/openFrameTypes";
...
export function MetalCladdings(props: Props) {
  ...
  const data = useMemo(() => {
    return openFrameUI?.metalCladdings ?? [];
  }, [openFrameUI]);
  ...
  function handleAddRow() {
    handleChangeData([...data, { id: getNextId(data), selected: false, elevation: 0, height: 0 }]);
  }
  ...
  function handleCreateElements(row: OFMetalCladdingUI, model: TOpenFrame) {
    ...
    const changedModel: TOpenFrame = {
      ...model,
      metalCladdings: model.metalCladdings ? [...model.metalCladdings, el] : [el],
    };
    return changedModel;
  }
  ...
  function handleDeleteModels(elements: OFMetalCladdingUI[], project?: Project) {
    ...
    for (const [key, ids] of Array.from(map.entries())) {
      ...
      for (const id of ids) {
        newModel = {
          ...newModel,
          metalCladdings: newModel.metalCladdings?.filter((item) => item.uiId !== id) ?? [],
        };
      }
      if (changedProject) {
        changedProject = {
