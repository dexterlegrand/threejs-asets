...
export function FPostFS(props: Props) {
  ...
  function handleHardChange(row: OFFPElementUI, field: string, value: any) {
    ...
    dispatch(changeModel(handleCreateAccessory(model, element as TFPElementOF, changed)));
  }

  function handleCreateAccessory(model: TOpenFrame, element: TFPElementOF, ui: OFFPElementUI) {
    let newModel = { ...model };
    let columns: string[] = [];
    let beams: string[] = [];
    ...
    importFromCSV((imported, isCSV) => {
      ...
      for (const row of imported as any[]) {
        ...
        if (changed) {
          ...
          if (rowP) {
            if (isNumber(rowP)) {
              changed = { ...changed, projection: Math.abs(rowP) };
            } else {
              showErrorMsg(
                `(id: ${row.id}) - a value of "Projection (m)" "${rowP}" is not a number!`
              );
            }
          }
