...
export function MasonryCladdings(props: Props) {
  ...
  const data = useMemo(() => {
    return openFrameUI?.masonryCladdings ?? [];
  }, [openFrameUI]);
  ...
  function handleAddRow() {
    handleChangeData([...data, { id: getNextId(data), selected: false, height: 0 }]);
  }
  ...
  function handleCreateElements(row: OFMasonryCladdingUI, model: TOpenFrame) {
    ...
    const changedModel: TOpenFrame = {
      ...model,
      masonryCladdings: model.masonryCladdings ? [...model.masonryCladdings, el] : [el],
    };
    return changedModel;
  }

  function handleChangeRow(row: OFMasonryCladdingUI, field: string, value: any) {
  ...
  function handleDeleteModels(elements: OFMasonryCladdingUI[], project?: Project) {
    ...
    for (const [key, ids] of Array.from(map.entries())) {
      ...
      for (const id of ids) {
        newModel = {
          ...newModel,
          masonryCladdings: newModel.masonryCladdings?.filter((item) => item.uiId !== id) ?? [],
        };
      }
      if (changedProject) {
        changedProject = {
