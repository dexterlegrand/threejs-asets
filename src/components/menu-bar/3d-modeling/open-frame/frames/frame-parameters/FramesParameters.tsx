...
export default React.memo(function FrameParameters(props: Props) {
  const {
    project,
    openFrameUI,
    setTab,
  ...
  function handleChangeChainage(row: OFFramesParametersUI, value: number) {
    ...
    if (!model || !openFrameUI) return;
    const prevFrame = [...model.frames]
      .sort((a, b) => b.id - a.id)
      .find((item) => item.name < row.frame)?.name;
    const prevBeams = prevFrame
      ? model.beams.filter((item) => {
          if (item.frame !== prevFrame) return false;
          return model.columns.some((column) => {
            if (column.frame !== row.frame) return false;
    ...
    for (const element of prevBeams) {
      if (getAdditionalElements(map, element, openFrameUI).length) {
        dispatch(
          addEventAction(
            `Change Chainage: Frame "${element.frame}" has additional elements!`,
            "danger"
          )
        );
        return;
    ...
    for (const element of mapToArray(map).filter(
      (element) => element.frame === row.frame
    )) {
      if (getAdditionalElements(map, element, openFrameUI).length) {
        dispatch(
          addEventAction(
  ...
  function getRowOFP(
    item: OFFramesParametersUI,
    prev?: OFFramesParametersUI,
    next?: OFFramesParametersUI
  ) {
    const wPercent = `${100 / 8}%`;
    const prevChainage =
      prev && prev.model === item.model && prev.chainage
        ? prev.chainage + 0.001
        : 0;
    const nextChainage =
      next && next.model === item.model && next.chainage
        ? next.chainage - 0.001
        : undefined;
    return (
      <tr key={`${item.model}-${item.frame}`}>
        ...
        <NumericCell
          ...
          value={item.chainage}
          onChange={(value) => {
            console.log(prevChainage, prev);
            console.log(item.chainage, value, item);
            console.log(nextChainage, next);
            handleChangeChainage(item, value);
          }}
              ...
              if (param.model === itemModel && param.frame === itemFrame) {
                ...
                if (itemST) {
                  if (supportTypes.includes(itemST)) {
                    ...
                    onError(
                      `${startMSG} (id: ${item.id}) - Incorrect support type "${itemTH}"!`
                    );
                  }
                }
                return newParam;
      ...
      <div className={"p-5"} style={{ display: display ? "block" : "none" }}>
        <div className={"small-table-container"}>
          <table className="table bg-gray">
            ...
            <tbody>
              {parameters?.map((item, i, arr) =>
                getRowOFP(item, arr[i - 1], arr[i + 1])
              )}
            </tbody>
          </table>
        </div>
