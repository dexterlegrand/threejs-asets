...
import { getMaxAndMinPoints } from "../../../../3d-models/openFrame";

type Props = {
  models: TOpenFrame[];
};

const PlatformsOF: FunctionComponent<Props> = (props) => {
  const { models } = props;
  ...
  function handleChangeModel(item: OFPlatformUI) {
    ...
    if (from && to && item.width && item.thickness) {
      ...
      changedModel = {
        ...changedModel,
        platforms: [...changedModel.platforms, changed],
      };
    }
  ...
  function handleChange(item: OFPlatformUI, field: string, value: any) {
    const changed =
      field === "distanceFromLeft"
        ...
        : { ...item, [field]: value };
    handleChangeUI(
      data.map((dataItem) => (dataItem.id === item.id ? changed : dataItem))
    );
...
export default React.memo(PlatformsOF);

export function getLeftDistances(
  ...
  for (const el of elements) {
    ...
    if (isX) {
      ...
      if (sz === ez) continue;
      if (
        (hardCheckRange(sz, minV.z, maxV.z) &&
          hardCheckRange(ez, minV.z, maxV.z)) ||
      ...
      if (sx === ex) continue;
      if (
        (hardCheckRange(sx, minV.x, maxV.x) &&
          hardCheckRange(ex, minV.x, maxV.x)) ||
...
export function getWidths(distances: number[], leftDistance: number) {
  const width: number[] = [];
