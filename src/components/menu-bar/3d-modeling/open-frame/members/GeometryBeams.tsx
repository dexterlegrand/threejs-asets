...
const GeometryBeams: FunctionComponent<Props> = (props) => {
  ...
  function handleRestoreElements(elements: OFBeamUI[], toReturn?: boolean) {
    ...
    for (const [modelName, modelElements] of Array.from(map.entries())) {
      ...
      for (const modelElement of modelElements) {
        ...
        if (item.type === "BEAM") {
          ...
          if (beam.secondType === "GENERAL") {
            profile =
              beam.direction === "X"
                ? model.frameTieProfile
                : model.frameBeamProfile;
          } else if (beam.secondType === "BtoB") {
            profile = openFrameUI.additionalBeams.beamToBeam.find(
              (btob) => btob.id === beam.uiId
            )?.profile;
          } else if (beam.secondType === "CtoB") {
            profile = openFrameUI.additionalBeams.columnToBeam.find(
              (ctob) => ctob.id === beam.uiId
            )?.profile;
          ...
          profile = openFrameUI.additionalBeams.cantilever.find(
            (cnt) => cnt.id === item.uiId
          )?.profile;
        } else if (item.type === "HORIZONTAL-BRACING") {
          profile = openFrameUI.additionalBeams.planBracings.find(
            (pb) => pb.id === item.uiId
          )?.profile;
        }
      ...
      changedProject = {
        ...changedProject,
        models: changedProject.models.map((cpm) =>
          cpm.name === model?.name ? model : cpm
        ),
      };
    }
    if (toReturn) {
  ...
  function handleChangeModelElement(
    row: OFBeamUI,
    ...
    if (model && element) {
      ...
      if (field === "profile") {
        if (element.type === "BEAM") {
          if ((element as TBeamOF).secondType === "BtoB") {
            dispatch(
              changeOFUIAction({
                ...openFrameUI!,
                additionalBeams: {
                  ...openFrameUI!.additionalBeams,
                  beamToBeam: openFrameUI!.additionalBeams.beamToBeam.map(
                    (el) =>
                      el.id === element.uiId ? { ...el, profile: value } : el
                  ),
                },
                members: {
                  ...openFrameUI!.members,
                  beams: openFrameUI!.members.beams.map((el) =>
                    el.id === row.id ? { ...el, profile: value } : el
                  ),
                },
              })
            );
          } else if ((element as TBeamOF).secondType === "CtoB") {
            dispatch(
              changeOFUIAction({
                ...openFrameUI!,
                additionalBeams: {
                  ...openFrameUI!.additionalBeams,
                  columnToBeam: openFrameUI!.additionalBeams.columnToBeam.map(
                    (el) =>
                      el.id === element.uiId ? { ...el, profile: value } : el
                  ),
                },
          ...
          } else if ((element as TBeamOF).secondType === "CtoC") {
            dispatch(
              changeOFUIAction({
                ...openFrameUI!,
                additionalBeams: {
                  ...openFrameUI!.additionalBeams,
                  columnToColumn: openFrameUI!.additionalBeams.columnToColumn.map(
                    (el) =>
                      el.id === element.uiId ? { ...el, profile: value } : el
                  ),
                },
        ...
        } else if (element.type === "CANTILEVER") {
          dispatch(
            changeOFUIAction({
              ...openFrameUI!,
              additionalBeams: {
                ...openFrameUI!.additionalBeams,
                cantilever: openFrameUI!.additionalBeams.cantilever.map((el) =>
                  el.id === element.uiId ? { ...el, profile: value } : el
                ),
              },
              members: {
        ...
        } else if (element.type === "HORIZONTAL-BRACING") {
          dispatch(
            changeOFUIAction({
              ...openFrameUI!,
              additionalBeams: {
                ...openFrameUI!.additionalBeams,
                planBracings: openFrameUI!.additionalBeams.planBracings.map(
                  (el) =>
                    el.id === element.uiId ? { ...el, profile: value } : el
                ),
              },
    ...
    return (
      <tr key={item.id}>
        ...
        <SelectorCell<Orientation>
          ...
          onSelect={(value) =>
            handleChangeModelElement(item, "orientation", value)
          }
          itemKey={(item) => item}
          itemLabel={(item) => `${item}`}
          filterable={false}
      ...
      newData.forEach((item: any) => {
          ...
          if (model) {
            ...
            if (itemElement) {
              ...
              if (element) {
                ...
                showErrorMsg(
                  `(id: ${item.id}) - an element "${itemElement}" not found!`
                );
              }
            }
          } else {
        ...
        if (itemOrientation) {
          if (orientations.includes(itemOrientation)) {
            ...
            showErrorMsg(
              `(id: ${item.id}) - Incorrect orientation value (${itemOrientation})!`
            );
          }
        }
