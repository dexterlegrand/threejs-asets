...
const initPlate: OFRBasePlateUI = {
  id: 0,
  selected: false,
...
export function RectangularBasePlateOF(props: Props) {
  const { models } = props;
  ...
  useEffect(() => {
    if (BCS.type !== "ODSM" || BCS.anchor !== "BP") return;
    if (BCS.item?.subtype === "rectangular") {
      const model = models.find((m) => m.name === BCS.item!.model);
      if (!model) return;
      const id = getNextId(plates);
      let rbpUI: OFRBasePlateUI = {
        ...initPlate,
        id,
        model: BCS.item.model,
        column: BCS.item.element,
      };
      rbpUI = { ...rbpUI, shearKeyDetails: setShearKeyDetails(rbpUI) };
      handleChangeData("rectangular", [...plates, rbpUI]);
      handleChangeModel(rbpUI);
    ...
    dispatch(
      changeModel({
        ...
        rectangularBP: model.rectangularBP.some((rbp) => rbp.id === changed.id)
          ? model.rectangularBP.map((rbp) =>
              rbp.id === item.id ? changed : rbp
            )
    ...
    map.forEach((ids, key) => {
      ...
      if (model) {
        dispatch(
          changeModel({
            ...model,
            rectangularBP: model.rectangularBP.filter(
  ...
  function handleAdd() {
    handleChangeData("rectangular", [
      ...plates,
      { ...initPlate, id: getNextId(plates) },
    ]);
  }
  ...
  function handleAddForAll() {
    ...
    if (!plate.model) {
      dispatch(
        addEventAction(
          `Base Plates: Select a model in the selected entry!`,
          "danger"
        )
      );
      return;
  ...
  function setShearKeyDetails(row: OFRBasePlateUI) {
    const model = models.find((m) => m.name === row.model);
    const column = model?.columns.find((c) => c.name === row.column);
  ...
  function handleChange(item: OFRBasePlateUI, field: string, value: any) {
    ...
    if (field === "column" && changed.shearResistedBy === "Shear Key") {
      ...
      changed = {
        ...changed,
        shearKeyDetails:
          value === "Shear Key" ? setShearKeyDetails(changed) : undefined,
      };
    }
    handleChangeData(
      "rectangular",
  ...
  function getRow(item: OFRBasePlateUI) {
    ...
    const columns =
      model?.columns.filter(
        (column) => column.startPos.y === model.baseElevation
      ) ?? [];
    ...
    return (
      <tr key={item.id}>
        ...
        <SelectorCell<string>
          items={convertToNamesArray(
            columns.filter((column) => {
              ...
              return !plates.some(
                (r) => r.model === item.model && r.column === column.name
              );
            })
          )}
        ...
        <NumericCell
          ...
          onChange={(value) =>
            handleChange(item, "firstRowFromCenter_L", value)
          }
        />
        <NumericCell
          className="w-100"
        ...
        <NumericCell
          ...
          onChange={(value) =>
            handleChange(item, "firstRowFromCenter_W", value)
          }
        />
        <NumericCell
          className="w-100"
        ...
        <SelectorCell<number>
          ...
          onSelect={(value) =>
            handleChange(item, "stiffenerAlongFlange", value)
          }
          itemKey={(item) => item}
          itemLabel={(item) => `${item}`}
        />
              ...
              onClick={() => {
                setDlg(
                  <ShearKeyDetails
                    ...
                    onChange={(val) =>
                      handleChange(item, "shearKeyDetails", val)
                    }
  ...
  function handleExport() {
    exportToCSV(plates, "OF Rectangular Base Plates");
  }

  function showErrorMsg(msg: string) {
    dispatch(
      addEventAction(`Rectangular Base Plates (Import): ${msg}`, "danger")
    );
  }
  ...
  function handleImport() {
    ...
    importFromCSV((newData, isCSV) => {
      ...
      newData.forEach((item: OFRBasePlateUI) => {
        ...
        if (model) {
          const bp = model.rectangularBP.find(
          ...
          <FormGroup className="no-m" label="Concrete Grade" inline>
            <SimpleSelector<string>
              ...
              onSelect={(concreteGrade) =>
                handleChangeData("concreteGrade", concreteGrade)
              }
              itemLabel={(item) => item}
