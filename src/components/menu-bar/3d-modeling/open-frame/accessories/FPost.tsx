...
import { OFFPElementUI } from "../../../../../store/ui/types";
...
export function FPostOF(props: Props) {
  const { project, models, libs, profiles } = props;

  const [data, setData] = useState<OFFPElementUI[]>([]);
  ...
  function mapData(models: TOpenFrame[]) {
    let newData: OFFPElementUI[] = [];
    models.forEach((model) => {
      model.accessories.forEach((ag) => {
        if (ag.type === "FP") {
          //@ts-ignore
          ag.elements.forEach((el: TFPElementOF) => {
            newData = [
              ...newData,
              {
                id: el.id,
  ...
  function handleChangeRow(row: OFFPElementUI, field: string, value: any) {
    setData(
      data.map((element) => {
  ...
  function handleDeleteElements(elements: OFFPElementUI[]) {
  ...
  function handleHardChange(row: OFFPElementUI, field: string, value: any) {
    ...
    dispatch(changeModel(handleCreateAccessory(model, element as TFPElementOF, changed, group)));
  }

  function handleCreateAccessory(
    model: TOpenFrame,
    element: TFPElementOF,
    ui: OFFPElementUI,
  ...
  function handleSoftChange(row: OFFPElementUI, field: string, value: any) {
    handleChangeRow(row, field, value);
    let model = getElementByName(models, row.model);
    const group = getElementByName(model?.accessories, row.group);
    const element = getElementByField(group?.elements, "id", row.id);
  ...
  function getRow(item: OFFPElementUI) {
    const width = `${100 / 14}%`;
    return (
      <tr key={item.id}>
        <CheckBoxCell
        ...
        <NumericCell
          ...
          onChange={(value) => handleHardChange(item, "projection", value)}
          style={{ width }}
        />
      </tr>
    );
  ...
  function handleExport() {
    exportToCSV(
      data.map((item) => ({
        ...
        "Projection (m)": fixNumberToStr(item.projection),
      })),
      "F-Post Accessories"
    );
  }
  ...
  function showErrorMsg(msg: string) {
    handleMsg(`F-Post Accessories (Import): ${msg}`, "danger");
  }
  ...
  function handleImport() {
    ...
    importFromCSV((imported, isCSV) => {
      ...
      const newData: OFFPElementUI[] = [...data];
      for (const row of imported as any[]) {
        const rowName = row["F-Post Group No."];
        if (!rowName) continue;
        ...
        if (changed) {
          ...
          const rowP = row["Projection (m)"];

          changed = {
            ...changed,
            height: checkImportedNumber(rowHeight) ?? 0,
            h1: checkImportedNumber(rowH1) ?? 0,
            h2: checkImportedNumber(rowH2) ?? 0,
            h3: checkImportedNumber(rowH3) ?? 0,
            h4: checkImportedNumber(rowH4) ?? 0,
            beamOrientation: (checkImportedNumber(rowBO) ?? 0) as Orientation,
            columnOrientation: (checkImportedNumber(rowCO) ?? 0) as Orientation,
            projection: checkImportedNumber(rowP) ?? 0,
          };
          ...
          const model = getElementByName(changedProject.models as TOpenFrame[], changed.model);
