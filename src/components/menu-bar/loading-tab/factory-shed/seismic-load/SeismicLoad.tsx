...
export function SeismicLoadFS({ onClose }: Props) {
  ...
  const data = useMemo(() => {
    return openFrameUI?.loadingsUI.seismicLoadsUI;
  }, [openFrameUI]);
  ...
  function handleGenerate() {
    ...
    //   );
    // });
    // requests.length && dispatch(changeRequestProgressAction("seismic"));
    ...
    //       console.log(value);
    //       if (value.data) {
    //         Object.values(value.data).forEach((item: any) => {
    //           seismicLoads = [
    //             ...seismicLoads,
    //             {
    //               id: seismicLoads.length,
    //               model: item.openFrameName,
    //               level: item.level,
    //               node: item.node,
    ...
    //   .catch((err) => {
    //     console.error(err);
    //     dispatch(addEventAction(`Seismic Weight: ${err.message}`, "danger"));
    //     handleChangeData("seismicLoads", []);
    //   });
  }
          ...
          {data?.seismicAnalysisMethod === "Response Spectrum" ? (
            ...
            <>
              <div className={"d-flex f-ai-center label-light bg-dark"}>
                ...
                <FormGroup className="no-m" label="FS. No." inline>
