...
export function ConnectionDetailsDlg({
  item,
  process,
  onChange,
  onClose,
  ...
  function handleDelete() {
    ...
    if (item.type === EProcessElementType.MIX) {
      ...
      let i = 1;
      points = points.map((p) => {
        if (p.connectionType === "START") return p;
        const pos = l_3 - l * i++;
        const newPoint = new Vector3(-l_2, pos).applyAxisAngle(
          new Vector3(1),
          deg45InRad
        );
      ...
      let i = 1;
      points = points.map((p) => {
        if (p.connectionType === "END") return p;
        const pos = l_3 - l * i++;
        const newPoint = new Vector3(l_2, pos).applyAxisAngle(
          new Vector3(1),
          deg45InRad
  ...
  function getConnectionType(
    ...
    if (element.type === EProcessElementType.MIX) {
      return "END";
    } else if (element.type === EProcessElementType.SPLIT) {
      return "START";
    } else return point.connectionType;
  }
  ...
  function handleClear() {
    ...
    const changed = {
      ...
      points: item.points.map((p) => {
        ...
        return {
          ...p,
          connectionType: getConnectionType(item, p),
          element: undefined,
        };
      }),
    };
    ...
    return (
      <tr key={row.id}>
        ...
        <SelectorCell<"START" | "END">
          ...
          itemKey={(item) => item}
          itemLabel={(item) => item}
          selected={row.connectionType}
          onSelect={(connectionType) =>
            handleChange({ ...row, connectionType })
          }
    ...
    importFromCSV((data, isCSV) => {
      ...
      for (const item of data) {
        ...
        const newRow: TProcessElementPoint = {
          ...
          connectionType: item["Connection Type"]?.trim()==="END"?"END":"START",
          element: item["Element"]?.trim()==='�'?"":item["Element"]?.trim(),
                ...
                <table ref={tableRef} className="table bg-gray">
                  <thead>
                    <tr>
                      ...
                      <th rowSpan={3}>Connection Type</th>
